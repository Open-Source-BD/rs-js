...
async function main() {
    ...
    const adults = await process(users, [
    ...
    const active = await process(users, [
    ...
    const northAmerica = await process(users, [
    ...
    const withA = await process(users, [
    ...
    const seniorEngineers = await process(users, [
        {
            op: 'filter',
            logic: 'and',
            conditions: [
    ...
    const creativeTeams = await process(users, [
        {
            op: 'filter',
            logic: 'or',
            conditions: [
    ...
    const page2 = await process(users, [
        { op: 'filter', conditions: [{ field: 'active', operator: 'eq', value: true }] }
    ], { offset: 2, limit: 3 });
